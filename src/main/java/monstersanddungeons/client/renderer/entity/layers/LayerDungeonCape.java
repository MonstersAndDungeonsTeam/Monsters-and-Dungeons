...
public class LayerDungeonCape implements LayerRenderer<AbstractClientPlayer>
{
    ...
    private static ResourceLocation devCape = new ResourceLocation(MonstersAndDungeons.MODID + ":textures/models/cape/dev_cape.png");

    public LayerDungeonCape(RenderPlayer playerRendererIn)
    {
    ...
    public void doRenderLayer(AbstractClientPlayer entitylivingbaseIn, float limbSwing, float limbSwingAmount, float partialTicks, float ageInTicks, float netHeadYaw, float headPitch, float scale)
    {
        if (entitylivingbaseIn.hasPlayerInfo() && !entitylivingbaseIn.isInvisible() && entitylivingbaseIn.isWearing(EnumPlayerModelParts.CAPE))
        {
            ...
            if (itemstack.getItem() != Items.ELYTRA || itemstack.getItem() != AllItems.WASP_CHEST)
            {
                GlStateManager.color(1.0F, 1.0F, 1.0F, 1.0F);
                this.playerRenderer.bindTexture(devCape);
